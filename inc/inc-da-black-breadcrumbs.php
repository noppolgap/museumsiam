	...
	while ($row = mysql_fetch_array($rs)) {
		...
		if (count($breadArr) == 0) {
			...
			if (nvl($row['IS_LAST_NODE'], 'Y') == 'Y')
				$breadArr[] = '<li ><a href="da-all-black.php?MID=' . $mid . '&CID=' . $row['CONTENT_CAT_ID'] . '&SCID=' . $row['SUB_CONTENT_CAT_ID'] . '">' . $row[$selectedColName] . '&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;</a></li>';
			else
				$breadArr[] = '<li ><a href="da-category-black.php?MID=' . $mid . '&CID=' . $row['CONTENT_CAT_ID'] . '&SCID=' . $row['SUB_CONTENT_CAT_ID'] . '">' . $row[$selectedColName] . '&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;</a></li>';
		}
