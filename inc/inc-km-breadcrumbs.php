...
function getSubCatBread($scid, $mid, &$breadArr) {
	...
	while ($row = mysql_fetch_array($rs)) {
		...
		if (count($breadArr) == 0) {
			...
			$breadArr[] = '<li ><a href="km.php?MID=' . $mid . '&CID=' . $row['CONTENT_CAT_ID'] . '&SCID=' . $row['SUB_CONTENT_CAT_ID'] . '">' . $row['SUB_CONTENT_CAT_DESC_LOC'] . '&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;</a></li>';
		}
...
function getCatBread($cid, $mid, &$breadArr) {
	...
	while ($row = mysql_fetch_array($rs)) {
		...
		if (count($breadArr) == 0) {
			...
			$breadArr[] = '<li ><a href="km.php?MID=' . $mid . '&CID=' . $row['CONTENT_CAT_ID'] . '">' . $row['CONTENT_CAT_DESC_LOC'] . '&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;</a></li>';
		}
...
function getModuleBread($mid, &$breadArr) {
	...
	while ($row = mysql_fetch_array($rs)) {
		...
		if (count($breadArr) == 0) {
			...
			$breadArr[] = '<li ><a href="km.php?MID=' . $row['MODULE_ID'] .  '">' . $row['MODULE_NAME_LOC'] . '&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;</a></li>';
		}
