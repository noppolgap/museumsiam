...
<div class="main-container">
	<div class="main-body marginC">
		...
		$navigateBackPage = 'main_department_view.php?secid='.$secid;
		$link = "department_people_add.php?secid=" . $secid.'&did='.$did;
		?>
		<div class="mod-body">
			<div class="buttonActionBox">
				<input type="button" value="เพิ่มบุคลากร" class="buttonAction emerald-flat-button" onclick="window.location.href = '<?=$link ?>'">
				<input type="button" value="ลบ" class="buttonAction alizarin-flat-button" onclick="deleteCheck();" data-pageDelete="department_people_action.php?delete">
				<input type="button" value="จัดเรียง" class="buttonAction peter-river-flat-button" onclick="orderPage('department_people_order.php?secid=<?=$secid ?>&did=<?=$did?>');">
			...
			?>
				...
				<div class="mod-body-inner-header">
					<div class="floatL titleBox">การจัดการแผนก <?=$departmentName ?></div>
				...
				<?php while($row = mysql_fetch_array($query)) { ?>
					<div class="Main_Content" data-id="<?=$row['ORG_ID'] ?>" >
						...
						<div class="floatL thumbContent">
							<a href="department_people_detail.php?secid=<?=$secid ?>&orgid=<?=$row['ORG_ID'] ?>&did=<?=$did?>" class="dBlock" <?=callOrgPictureAdmin($row['IMG_PATH']) ?> ></a>
						</div>
						<div class="floatL nameContent">
							<div><? echo '<a href="department_people_detail.php?secid='.$secid.'&orgid='.$row['ORG_ID'].'&did='.$did.'">'. $row['NAME_LOC'].'</a>' ?></div>
						...
						<div class="floatL EditContent">
							<a href="department_people_edit.php?secid=<?=$secid ?>&orgid=<?=$row['ORG_ID'] ?>&did=<?=$did?>" class="EditContentBtn">Edit</a>
