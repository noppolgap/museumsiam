			...
			<div class="mod-body-inner">
				...
				<div class="mod-body-main-content">
					<div class="formCms">
						<? $id = $_GET['g']; ?>
						<?php
							   $sql= "SELECT * FROM trn_sub_digital_ach d join trn_main_digital_ach s on
									  s.MAIN_DIGITAL_ID = d.SUB_DIGITAL_ID WHERE d.FLAG <> 2 AND d.SUB_DIGITAL_ID = $id";
							   $query = mysql_query($sql,$conn);
						?>

						<form action="sub_digital_action.php?edit&p=<?=$id?>" method="post" name="formcms">

						<? while($row = mysql_fetch_array($query)) {  ?>
							<div>
								<div class="floatL form_name">หมวดหมู่</div>
								<input type="hidden" name="sub_id" value="<? echo $id; ?>" class="w90p" />
								<input type="hidden" name="main_id" value="<? echo $row['MAIN_DIGITAL_ID'];?>" class="w90p" />
							...
							<div>
								...
								<div class="floatL form_input"><input type="text" name="sub_name_th" value="<? echo $row['SUB_DIGITAL_DESC_LOC']; ?>" class="w90p" /></div>
								<div class="clear"></div>
							</div>
							<div>
								<div class="floatL form_name">ชื่อ EN</div>
								<div class="floatL form_input"><input type="text" name="sub_name_en" value="<? echo $row['SUB_DIGITAL_DESC_ENG']; ?>" class="w90p" /></div>
								<div class="clear"></div>
							</div>
